...
- **KOSIS_KEY (권장)**: [KOSIS 공유서비스](https://kosis.kr/openapi/) 인증키 신청(무료) → `KOSIS_KEY` 등록. 시도별 아파트 호수(≈세대수)를 **집계로 1콜/시도** 수집(쿼터벽 없음). `--kosis-stock`.
  - 통계표 코드는 표마다 달라 첫 실행 로그의 `첫 레코드`로 확정하며, 필요 시 `KOSIS_APT_ORG/TBL/ITM/OBJL1/OBJL2` env로 교정한다(기본: 인구주택총조사 주택종류별).
