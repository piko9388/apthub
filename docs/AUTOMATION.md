...
### 4) 공공임대 비율의 '분모'(전체 아파트 세대수) *선택*
분모는 **집계 통계(KOSIS)**로 얻는 것이 견고하다. K-apt 단지별 조회는 수도권(1만+ 단지) > 일 쿼터(1만)라 **HTTP 429로 전수 불가**임이 실측 확인됐다.

- **수동 시드(현재 권장)**: `data/seed/apt-stock.json`에 시도별 아파트 세대수(호)를 **공식 발표값(통계청 인구주택총조사)**으로 직접 적재. 아파트 재고는 **연간** 통계라 상수처럼 유지해도 무방하며, 연 1회 갱신이면 충분하다. 형식은 지표 시드와 동일(`metric:"아파트 세대수", sido, value, unit:"세대", date, confidence:"공식", url:<통계청>`).
  - ⚠️ **KOSIS 자동수집은 GitHub Actions(해외 러너)에서 kosis.kr 접속이 타임아웃**되어 실패한다(실측 확인). CI 환경 밖(국내)에서 `--kosis-stock`으로 시드를 생성해 커밋하는 방식은 가능. `KOSIS_APT_ORG/TBL/ITM/OBJL1/OBJL2` env로 표 조정.
- **(대안) KAPT_STD_UDDI + KAPT_KEY**: [전국공동주택표준데이터 #15096285](https://www.data.go.kr/data/15096285/standard.do) 벌크 — 요청 URL의 `uddi:` 값이 있을 때 `--kapt`.
- **(비권장) 단지별 API**: #15058453/#15057332 조합(`--kapt-api`)은 429 쿼터벽으로 수도권 전수 불가. 운영계정(쿼터 상향) 때만.

등록 후 빌드가 `공공임대 비율(%) = 임대 세대수 ÷ 아파트 세대수`를 시도별 산출(○추정). 워크플로는 `KOSIS_KEY`가 있으면 KOSIS, 없고 `KAPT_STD_UDDI`가 있으면 벌크로 자동 동작한다.
...
- `KOSIS_KEY` = (KOSIS 인증키 — 공공임대 비율 분모, 권장)
