...
- **경로 A — 목록+기본정보 API(권장, uddi 불필요)**: [공동주택 단지 목록제공 #15057332](https://www.data.go.kr/data/15057332/openapi.do)와 [공동주택 기본 정보제공 #15058453](https://www.data.go.kr/data/15058453/openapi.do) 둘 다 활용신청 → `KAPT_KEY`에 인증키 등록. 목록으로 수도권 단지코드를 열거하고 단지마다 세대수(kaptdaCnt)를 조회해 시군구별 합산. **단지당 1콜**이라 수도권 수천 콜(일 쿼터 1만 내). `--kapt-api`.
- **경로 B — 벌크 표준데이터(콜 수 최소, uddi 필요)**: [전국공동주택표준데이터 #15096285](https://www.data.go.kr/data/15096285/standard.do) 활용신청 → `KAPT_KEY`+`KAPT_STD_UDDI`(요청 URL의 `uddi:` 값). 한 파일로 전 아파트 세대수(~16콜). `--kapt`.

둘 다 K-apt 원천이라 호갱노노·네이버 아파트 세대수와 동일 계보이며 전수·약관 문제 없음. 등록 후 빌드가 `공공임대 비율(%) = 임대 세대수 ÷ 아파트 세대수`를 시도별 산출(○추정).
`KAPT_STD_UDDI`가 있으면 벌크(B), 없으면 목록+기본정보(A)로 자동 동작한다.
