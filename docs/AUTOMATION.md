...
### 4) KAPT_KEY (공공임대 비율의 '분모' — 전국공동주택표준데이터) *선택*
1. <https://www.data.go.kr/data/15096285/standard.do> "전국공동주택표준데이터" 활용신청(자동승인)
2. 발급 인증키를 `KAPT_KEY`로 등록. 추가로 포털의 **요청 URL(`.../uddi:xxxx`)에서 uddi 값**을 `KAPT_STD_UDDI`로 등록
3. → 수도권 시군구별 **아파트 세대수 전수(분모)**를 수집하고, 빌드가 `공공임대 비율(%) = 임대 세대수 ÷ 아파트 세대수`를 시도별로 산출(○추정 표기)
   - 이 데이터가 호갱노노·네이버 아파트 세대수의 원천(K-apt)이며 전수·약관 문제 없음.
...
- `KAPT_KEY` = (전국공동주택표준데이터 인증키, 선택) · `KAPT_STD_UDDI` = (요청 URL의 uddi)
